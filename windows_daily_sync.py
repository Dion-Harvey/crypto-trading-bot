#!/usr/bin/env python3
"""
Windows-Compatible Daily Trade Log Sync
    ...
    def run_fetch_trades(self):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                # Don't log the full output to avoid emoji encoding issues
                logger.info("[INFO] Trade fetch output logged to file")
            else:
                logger.error(f"[ERROR] Trade log fetch failed with return code {result.returncode}")
                if result.stderr:
                    logger.error(f"[ERROR] Error details: {result.stderr}")
                return False
    ...
    def run_sync_logs(self):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                # Don't log the full output to avoid emoji encoding issues
                logger.info("[INFO] Sync output logged to file")
            else:
                logger.error(f"[ERROR] Log synchronization failed with return code {result.returncode}")
                if result.stderr:
                    logger.error(f"[ERROR] Error details: {result.stderr}")
                return False
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        print("Usage: python windows_daily_sync.py [--test|--schedule|--help]")
