...
try:
    ...
    if phase2_validation['status'] in ['valid', 'valid_with_warnings']:
        ...
        log_message(f"🎯 Features: Exchange flows, whale tracking, DeFi intelligence, DEX analytics")
    ...
    for attempt in range(max_retries):
        try:
            ...
            # 🚀 ENHANCED RATE LIMITING DETECTION AND HANDLING
                ...
                log_message(f"   🔧 API Optimization: Consider reducing scan frequency")
    ...
    if ONCHAIN_AVAILABLE:
        try:
            ...
            if enhanced_score > emergency_threshold:
                ...
                return True, f"🚨 ENHANCED SPIKE DETECTED: {selected_crypto['symbol']} (tech: {selected_crypto['score']:.3f} + onchain: {onchain_score:.3f} = {enhanced_score:.3f}) - {factor_text}"
            
            # 🎯 ON-CHAIN PREDICTIVE SWITCHING: Early detection before technical signals peak
            if signal_strength in ['very_strong', 'strong'] and selected_crypto['score'] > 0.50:
                factors = onchain_analysis.get('contributing_factors', [])
...
def place_intelligent_order(symbol, side, amount_usd, use_limit=True, timeout_seconds=None, force_maker=False):
    ...
    try:
        ...
        if use_limit:
                ...
                if not filled:
                    if force_maker:
                        ...
                        try:
                            ...
                            log_message(f"⏰ POST-ONLY order timeout - cancelled (preserving maker fees)")
                            print(f"⏰ Post-only order timeout - order cancelled")
                        ...
                        try:
                            safe_api_call(exchange.cancel_order, order_id, symbol)
                            log_message(f"⏰ Limit order timeout - falling back to market order")
            ...
            log_message(f"⚡ MARKET ORDER: Will incur 0.1% taker fees")
            order = safe_api_call(exchange.create_market_order, symbol, side.lower(), amount)
            final_price = market_price
            if order:
            ...
            try:
                ...
                if crypto_purchased > 0:
                    ...
                    if trailing_stop:
                        ...
                        log_message(f"🎯 TRAILING STOP PROTECTION: Simple and effective protection active")
                        log_message(f"   🛡️ Trail Distance: {trail_pct:.1f}%")
                        log_message(f"   📈 Protected Amount: {crypto_purchased:.6f} {crypto_symbol}")
                        log_message(f"   🔄 Auto-trailing: Will follow price up automatically")
                        
                        print(f"✅ TRAILING STOP PROTECTION ACTIVE")
                        print(f"   🎯 Trail Distance: {trail_pct:.1f}%")
                        print(f"   🔄 TRAILING: Automatically follows price up")
                        print(f"   📊 Type: Binance native trailing stop")
...
def place_immediate_stop_limit_order(symbol, entry_price, btc_amount):
    """
    ...
    try:
        ...
        if not risk_config.get('immediate_stop_limit_enabled', False):
            log_message("🔒 Immediate stop-limit disabled in config")
            return None
        
        # Validate inputs
        if not symbol or not entry_price or not btc_amount:
            log_message(f"❌ Invalid stop-limit inputs: symbol={symbol}, price={entry_price}, amount={btc_amount}")
            return None
            
        if btc_amount <= 0 or entry_price <= 0:
            log_message(f"❌ Invalid stop-limit values: amount={btc_amount}, price=${entry_price}")
            return None
            
        # Get stop-limit percentage (default -0.125%)
        stop_limit_pct = risk_config.get('immediate_stop_limit_pct', 0.00125)  # 0.125%
        
        # Calculate stop-limit prices
        stop_price = entry_price * (1 - stop_limit_pct)          # Trigger price
...
def place_simple_trailing_stop(symbol, entry_price, btc_amount, current_price):
    """
    🎯 SIMPLE TRAILING STOP - The Only Exit Mechanism
    
    Places a Binance native trailing stop order immediately after buy.
    Clean, simple, and effective for day trading micro profits.
    """
    global trailing_stop_order_id, trailing_stop_active
    
    try:
        # Calculate trailing stop parameters
        trailing_distance_pct = optimized_config['risk_management'].get('trailing_stop_pct', 0.005)  # 0.5% default
        
        # Dynamic trailing distance based on volatility
        try:
            # Get recent volatility
            recent_data = fetch_ohlcv(exchange, symbol, '5m', 20)
            if len(recent_data) >= 10:
                ...
                if volatility > 0.02:  # High volatility
                    ...
                    trailing_distance_pct = 0.006  # 0.6%
                else:  # Low volatility
                    trailing_distance_pct = 0.004  # 0.4%
                    
                log_message(f"📊 Volatility-adjusted trailing distance: {trailing_distance_pct*100:.1f}% (vol: {volatility*100:.1f}%)")
        except:
            log_message("⚠️ Using default trailing distance")
        
        # Validate order size - Reduced to Binance minimum
        MIN_NOTIONAL_VALUE = 10.0  # Binance minimum is $10, not $11
        ...
        if order_value < MIN_NOTIONAL_VALUE:
            ...
            # Try placing a simple stop-market instead for small orders
            return place_simple_stop_market(symbol, entry_price, btc_amount, current_price)
        
        # Try to place Binance native trailing stop
        try:
            # Use Binance's built-in trailing stop - FIXED for Binance US API requirements
            log_message(f"🎯 PLACING NATIVE TRAILING STOP: {trailing_distance_pct*100:.1f}% callback rate")
            
            # Calculate activation price (current market price)
            ...
            order = safe_api_call(
                lambda: exchange.create_order(
                    ...
                    'sell',
                    btc_amount,
                    None,  # No price for trailing stop market orders
                    {
                        'callbackRate': str(trailing_distance_pct * 100),  # Convert to percentage (e.g., 0.5 for 0.5%)
                        'activationPrice': str(activation_price),  # Required activation price
                        'timeInForce': 'GTC'
                    }
                )
            )
            ...
            if order and order.get('id'):
                ...
                log_message(f"✅ TRAILING STOP PLACED: 0.50% trail distance (Native Binance)")
                ...
                log_message(f"   Activation Price: ${activation_price:.2f}")
                
                print(f"✅ TRAILING STOP ACTIVE: 0.50% trail distance")
                print(f"   Protection locked in - will follow price up automatically")
                
                return {
                    'order_id': trailing_stop_order_id,
                    'trail_distance': 0.005,  # 0.50%
                    'callback_rate': '0.5',
            ...
            log_message(f"⚠️ Native trailing stop failed: {native_error}")
            
            # Fallback to stop-limit order
            try:
                stop_price = current_price * (1 - trailing_distance_pct)
                ...
                order = safe_api_call(
                    lambda: exchange.create_order(
                        ...
                        'sell',
                        btc_amount,
                        None,  # No limit price for stop market orders
                        None,  # No stop price as positional argument
                        {
                            'stopPrice': stop_price,  # Stop price in params
                            'timeInForce': 'GTC', 
                            'type': 'STOP_MARKET'
                        }
                    )
                ...
                if order and order.get('id'):
                    ...
                    log_message(f"✅ STOP-LIMIT FALLBACK: Stop=${stop_price:.2f}, Limit=${limit_price:.2f}")
                    log_message(f"   Order ID: {trailing_stop_order_id}")
                    
                    print(f"✅ STOP-LIMIT PROTECTION: ${stop_price:.2f}")
                ...
                log_message(f"❌ Stop-limit fallback failed: {fallback_error}")
                return None
...
def place_simple_stop_market(symbol, entry_price, btc_amount, current_price):
    """
    🛡️ SIMPLE STOP MARKET - Fallback for small orders
    
    Places a basic stop-market order for orders too small for trailing stops.
    """
    try:
        # Calculate stop price (0.5% below current price for protection)
        stop_price = current_price * 0.995  # 0.5% stop loss
        
        log_message(f"🛡️ PLACING SIMPLE STOP-MARKET (small order fallback):")
        log_message(f"   Entry: ${entry_price:.2f}, Current: ${current_price:.2f}")
        log_message(f"   Stop Price: ${stop_price:.2f} (-0.5%)")
        log_message(f"   Amount: {btc_amount:.6f} {symbol.split('/')[0]}")
        ...
        order = safe_api_call(
            lambda: exchange.create_order(
                ...
                'sell',
                btc_amount,
                None,  # No price for stop market orders
                {
        ...
        if order and order.get('id'):
            log_message(f"✅ STOP-MARKET PLACED: Order ID {order['id']}")
            ...
            log_message("❌ Stop-market order failed")
            return None
            
    except Exception as e:
        log_message(f"❌ Stop-market fallback failed: {e}")
        return None
    ...
    try:
        ...
        if golden_cross:
            ...
            return {
                ...
                'confidence': 1.0,
                'reasons': [
                    f"Death Cross: EMA7 crossed below EMA25",
                    f"EMA7 prev: {ema7_previous:.4f} → {ema7_current:.4f}",
                    f"EMA25 prev: {ema25_previous:.4f} → {ema25_current:.4f}",
                    f"Spread: {ema_spread:.2f}%"
                ],
        ...
        return {'action': 'HOLD', 'confidence': 0.0, 'reasons': [f'EMA crossover error: {e}'], 'crossover_type': 'error'}
